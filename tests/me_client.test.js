...
describe("ME Client native adapter", () => {
  ...
  test("defines integrated client chrome and compositor-owned platform window shaping", () => {
    ...
    expect(runtime).not.toContain('"#login-screen, .sidebar-heading, .view-tabs"');
    ...
    expect(css).toContain("html.me-client-platform-windows {");
    ...
    expect(css).not.toContain("html.me-client-platform-macos body,");
    ...
    expect(native).toContain("GetWindowRect");
    expect(native).toContain("GetDpiForWindow");
    expect(native).toContain("WINDOWS_CORNER_RADIUS_CSS_PX: i32 = 8");
    expect(native).toContain("WINDOWS_WINDOW_SQUARE.store(!floating, Ordering::Release)");
    expect(native).toContain("(WINDOWS_CORNER_RADIUS_CSS_PX * dpi + 48) / 96");
    expect(native).toContain("WINDOWS_SUBCLASS_ID");
