...
describe("ME Client native adapter", () => {
  ...
  test("locks one product version and one complete package per release target", () => {
    ...
    expect(release).toContain("LINUX_BUILD_LABEL=studio.lytsing.me-s.release=linux-build");
    expect(release).toContain('docker ps --all --quiet --filter "label=$LINUX_BUILD_LABEL"');
    expect(release).toContain("trap cleanup_release_linux_on_exit EXIT");
    expect(linuxBuilder).toContain("docker run --rm");
    expect(linuxBuilder).toContain('tar -cf "$WORK/source.tar"');
    expect(linuxBuilder).toContain('--platform "$PLATFORM"');
    expect(linuxBuilder).toContain('--volume "$WORK/source.tar:/input/source.tar:ro"');
    ...
    expect(release + linuxBuilder).not.toContain("docker buildx build");
    expect(linuxDockerfile).toContain("build-in-container.sh");
    expect(linuxInContainer).toContain("cargo zigbuild");
    expect(linuxInContainer).toContain("build --no-bundle");
    expect(linuxInContainer).toContain("bundle --verbose --bundles appimage");
