            ...
            .spawn(move || {
                ...
                while !inside_close_trigger.load(Relaxed) {
                    let result = server
                        .accept()
                        .and_then(|(sock, _)| {
                            // Bound socket inactivity, not application execution time or total transfer time.
                            sock.set_timeouts(Duration::from_secs(30))?;
                            use util::RefinedTcpStream;
                            let (read_closable, write_closable) = match ssl {
                                None => RefinedTcpStream::new(sock),
                                #[cfg(any(feature = "ssl-openssl", feature = "ssl-rustls"))]
                                Some(ref ssl) => RefinedTcpStream::new(
                                    ssl.accept(sock)
                    ...
                    match result {
                        ...
                        Err(e) => {
                            // Retain the listener and report once per failure burst. No request is replayed.
                            let key = (e.kind(), e.raw_os_error());
                            if last_error != Some(key) {
                                log::error!("Error accepting new client; retrying: {}", e);
