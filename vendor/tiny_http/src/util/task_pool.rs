...
impl TaskPool {
    pub fn new() -> std::io::Result<TaskPool> {
        let pool = TaskPool {
    ...
    pub fn spawn(&self, code: Box<dyn FnMut() + Send>) -> std::io::Result<()> {
    ...
    fn add_thread(&self, initial_fn: Option<Box<dyn FnMut() + Send>>) -> std::io::Result<()> {
        let sharing = self.sharing.clone();

        thread::Builder::new()
            .name("tiny-http-connection".into())
            .spawn(move || {
                let sharing = sharing;
