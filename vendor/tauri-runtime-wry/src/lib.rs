...
fn create_window<T: UserEvent, F: Fn(RawWindow) + Send + 'static>(
  ...
  #[cfg(windows)]
  let uses_no_redirection_bitmap =
    is_window_transparent && !window_builder.inner.window.decorations;
  ...
  #[cfg(windows)]
  if uses_no_redirection_bitmap {
    // This Tao option only works when present before the HWND is created.
  ...
  #[cfg(windows)]
  let surface = if is_window_transparent && !uses_no_redirection_bitmap {
    if let Ok(context) = softbuffer::Context::new(window.clone()) {
      if let Ok(mut surface) = softbuffer::Surface::new(&context, window.clone()) {
        window.draw_surface(&mut surface, background_color);
        Some(surface)
