...
pub(super) struct WindowsRemoteControlBackend {
    geometry: Option<ScreenGeometry>,
}

impl WindowsRemoteControlBackend {
    ...
    fn geometry(&mut self) -> RemoteResult<ScreenGeometry> {
        let geometry = primary_geometry()?;
        self.geometry = Some(geometry);
        Ok(geometry)
    }

    fn current_geometry(&self) -> RemoteResult<ScreenGeometry> {
        self.geometry.ok_or_else(|| {
            RemoteControlError::input("remote-control display geometry is unavailable")
        })
    }

    fn capture_rgba(&self, geometry: ScreenGeometry) -> RemoteResult<Vec<u8>> {
        let width = i32::try_from(geometry.pixel_width)
            .map_err(|_| RemoteControlError::capture("Windows display width is too large"))?;
        let height = i32::try_from(geometry.pixel_height)
            .map_err(|_| RemoteControlError::capture("Windows display height is too large"))?;
        let byte_count = usize::try_from(geometry.pixel_width)
            .ok()
            .and_then(|width| {
                usize::try_from(geometry.pixel_height)
                    .ok()
                    .and_then(|height| width.checked_mul(height))
            })
            .and_then(|pixels| pixels.checked_mul(4))
            .ok_or_else(|| {
                RemoteControlError::capture("Windows screenshot buffer size overflowed")
            })?;
        let size_image = u32::try_from(byte_count).map_err(|_| {
            RemoteControlError::capture("Windows screenshot exceeds the GDI DIB size limit")
        })?;
        ...
        if copied_lines != height {
            return Err(RemoteControlError::capture(format!(
                "Windows GetDIBits returned {copied_lines} of {height} scan lines (Win32 error {})",
                last_error_code()
            )));
        }
        if pixels
        ...
        {
            return Err(RemoteControlError::interactive_desktop(
                "Windows returned a completely black primary-display frame; unlock the active desktop and try again",
            ));
        }
    ...
    fn send_inputs(&self, inputs: &[Input]) -> RemoteResult<()> {
        ...
        if inserted == count {
            ...
            Err(RemoteControlError::input(format!(
                "Windows SendInput inserted {inserted} of {count} remote events (Win32 error {}); the desktop may have changed or UIPI may be blocking input",
                last_error_code()
            )))
        }
    }
    ...
    fn post_key(&self, key: &str, down: bool) -> RemoteResult<()> {
        let mapping = key_mapping(key).ok_or_else(|| {
            RemoteControlError::input(format!("Windows has no remote key mapping for {key:?}"))
        })?;
        self.send_inputs(&[keyboard_input(mapping, down)])
    }
}

impl NativeRemoteControlBackend for WindowsRemoteControlBackend {
    ...
    fn capture(&mut self) -> RemoteResult<(ScreenGeometry, RgbaImage)> {
        ...
        let pixels = match self.capture_rgba(geometry) {
            Ok(pixels) => pixels,
            Err(error) => {
                ensure_interactive_desktop()?;
                return Err(error);
        ...
        ensure_interactive_desktop()?;
        let image = RgbaImage::from_raw(geometry.pixel_width, geometry.pixel_height, pixels)
            .ok_or_else(|| {
                RemoteControlError::capture("Windows returned invalid remote screenshot pixels")
            })?;
        Ok((geometry, image))
    }
    ...
    fn mouse_move(&mut self, x: u32, y: u32, _held: &[RemoteMouseButton]) -> RemoteResult<()> {
        ...
        self.send_inputs(&[mouse_input(
            absolute_coordinate(x, geometry.pixel_width),
            absolute_coordinate(y, geometry.pixel_height),
            0,
    ...
    fn text(&mut self, text: &str) -> RemoteResult<()> {
        ...
        for unit in text.encode_utf16() {
            ...
            if inserted != count {
                if inserted == 1 {
                    let release = unicode_input(unit, false);
                    unsafe {
                        SendInput(
                            1,
                            &release,
                ...
                return Err(RemoteControlError::input(format!(
                    "Windows SendInput inserted {inserted} of {count} remote Unicode events (Win32 error {})",
                    last_error_code()
                )));
            }
        }
...
fn primary_geometry() -> RemoteResult<ScreenGeometry> {
    let width = unsafe { GetSystemMetrics(SM_CXSCREEN) };
    let height = unsafe { GetSystemMetrics(SM_CYSCREEN) };
    if width <= 0 || height <= 0 {
        return Err(RemoteControlError::interactive_desktop(format!(
            "Windows returned invalid primary-display geometry {width}x{height}"
        )));
    }
    let pixel_width = u32::try_from(width)
        .map_err(|_| RemoteControlError::capture("Windows display width is invalid"))?;
    let pixel_height = u32::try_from(height)
        .map_err(|_| RemoteControlError::capture("Windows display height is invalid"))?;
    Ok(ScreenGeometry {
        pixel_width,
        pixel_height,
        logical_x: 0.0,
...
fn ensure_interactive_desktop() -> RemoteResult<()> {
    let station = unsafe { GetProcessWindowStation() };
    if station == 0 {
        return Err(RemoteControlError::interactive_desktop(last_error_message(
            "Windows could not read the process Window Station",
        )));
    }
    let station_name = user_object_name(station).map_err(|message| {
        RemoteControlError::interactive_desktop(format!(
            "Windows could not identify the process Window Station: {message}"
        ))
    })?;
    if !station_name.eq_ignore_ascii_case("WinSta0") {
        return Err(RemoteControlError::interactive_desktop(format!(
            "Remote control requires WinSta0, but this process is attached to {station_name:?}"
        )));
    }
    ...
    let name = user_object_name(desktop.0).map_err(|message| {
        RemoteControlError::interactive_desktop(format!(
    ...
    if !name.eq_ignore_ascii_case("Default") {
        return Err(RemoteControlError::interactive_desktop(format!(
            "Remote control requires the normal Default input Desktop, but Windows is using {name:?}"
        )));
    }
    Ok(())
}
...
fn mouse_button_flag(button: RemoteMouseButton, down: bool) -> u32 {
    match (button, down) {
...
impl ScreenDc {
    fn acquire() -> RemoteResult<Self> {
        let dc = unsafe { GetDC(0) };
        if dc == 0 {
            Err(RemoteControlError::capture(last_error_message(
...
impl MemoryDc {
    fn create(compatible_with: Hdc) -> RemoteResult<Self> {
...
impl Bitmap {
    fn create(dc: Hdc, width: i32, height: i32) -> RemoteResult<Self> {
...
impl SelectedObject {
    fn select(dc: Hdc, object: HGdiObject) -> RemoteResult<Self> {
    ...
    fn restore(&mut self) -> RemoteResult<()> {
        if !self.active {
            return Ok(());
        }
        let result = unsafe { SelectObject(self.dc, self.previous) };
        if result == 0 || result == HGDI_ERROR {
            return Err(RemoteControlError::capture(last_error_message(
...
mod tests {
    ...
    #[test]
    fn remote_win32_ffi_layouts_match_the_platform_abi() {
    ...
    #[test]
    fn remote_absolute_coordinates_cover_the_primary_screen() {
        assert_eq!(absolute_coordinate(0, 1920), 0);
        assert_eq!(absolute_coordinate(1919, 1920), 65_535);
    }

    #[test]
    fn all_remote_dom_keys_have_windows_mappings() {
        for key in [
            "return",
            "escape",
            "space",
            ...
            assert!(
                key_mapping(key).is_some(),
                "missing Windows remote key mapping for {key}"
            );
        }
        for byte in b'a'..=b'z' {
    ...
    #[test]
    fn remote_bgra_conversion_produces_opaque_rgba() {
