...
"""ME-S default Desktop toolbox.

This program is a persistent JSONL toolbox process. Python 3.12 hosts
the process while ME-S's desktop worker supplies validated host-desktop
capture and input operations.
"""
...
if sys.version_info[:2] != (3, 12):
    fail(
        "Desktop toolbox requires Python 3.12; "
        f"received {sys.version_info.major}.{sys.version_info.minor}"
    )

host = os.environ.get("ME_TOOLBOX_HOST") or shutil.which("me-s")
if not host:
    fail("Desktop toolbox cannot find ME-S; set ME_TOOLBOX_HOST or install me-s")

worker = subprocess.Popen(
    [host, "__toolbox-desktop-worker"],
...
if worker.stdin is None or worker.stdout is None or worker.stderr is None:
    fail("Desktop toolbox could not open worker pipes")
