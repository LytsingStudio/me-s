...
ELEMENT_ID = {
    ...
    "description": "An ARIA reference copied exactly from [ref=…] in the latest text snapshot. Frame elements use values such as f1e2.",
}
...
SNAPSHOT_SCHEMA = object_schema(
    {
        ...
        "accessibility_tree": {
            "type": ["string", "object"],
            "description": "Unmodified browser ARIA snapshot in AI mode. Present for text and both. A model-context safety crop may return aria_fragments with exact source line ranges; an oversized single text region may use text_fragments with exact byte ranges.",
        },
        "screen_path": {
            "type": "string",
...
INSTRUCTIONS = {
    ...
    "Close": "Closing a page invalidates all of its element IDs. Closing the last page keeps the shared browser session alive so Create can open another page.",
}
